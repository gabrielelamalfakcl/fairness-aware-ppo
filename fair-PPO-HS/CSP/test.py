...
from PPOAgentMDCSP import PPOAgentMDCSP
from environments.Hospital.utils import generate_arrival_templates

def test_ppo_csp(agents: dict, config: dict, test_rng: random.Random, env: HospitalMAS): 
    """
    Test of Fair-PPO (CSP)
    """
    ...
    print(f"Running {config['num_test_episodes']} test episodes for CSP model...")
            ...
            with torch.no_grad():
                for agent_name, agent_obs in obs.items():
                    if agent_obs.sum() > 0:
                        ...
                        action, _, _ = agents[agent_name].select_actions(state_tensor, use_greedy=False)
    ...
    output_filename = f"fairness_results_csp_alpha={config['alpha']}_beta={config['beta']}.pkl"
    output_path = os.path.join(config["output_dir"], output_filename)
    df.to_pickle(output_path)

    print(f"\nTest results DataFrame for CSP model saved to {output_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run Fair PPO (CSP) testing for Hospital MAS")
    ...
    parser.add_argument("--output_dir", type=str, default="fairness_results_CSP", help="Root directory to save the test result files.")
    ...
    for name, input_dim in agent_input_dims.items():
        agents[name] = PPOAgentMDCSP(input_dim=input_dim, action_space=main_env.action_spaces[name], lr=0, gamma=0, eps_clip=0, k_epochs=0, batch_size=0, alpha=0, beta=0, value_loss_coef=0, entropy_coef=0, device=config["device"])
        weights_path = os.path.join(config['weights_dir'], f'{name}_weights.pth')
    ...
    test_ppo_csp(agents, config, test_rng, main_env) 
