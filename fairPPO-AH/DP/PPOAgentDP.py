...
class PPOAgentDP(PPOAgent):
    """
    PPOAgentDP extends the standard PPOAgent to include a demographic parity penalty in its loss function
    """
        ...
        for _ in range(self.k_epochs):
            for start in range(0, len(memory_states), self.batch_size):
                ...
                with autocast():
                    ...
                    scaled_penalty = lambda_dp * combined_dp_penalty # penalty scaled with lambda to compare with loss
    ...
    def update(self, memory_able, memory_disabled):
        ...
        loss_able, ppo_loss_able, scaled_penalty_able, lambda_dp_able = self._update_memory(memory_able, memory_disabled)
        loss_disabled, ppo_loss_disabled, scaled_penalty_disabled, lambda_dp_disabled = self._update_memory(memory_disabled, memory_able)

        combined_loss = loss_able + loss_disabled

        return combined_loss, loss_able, loss_disabled, ppo_loss_able, ppo_loss_disabled, scaled_penalty_able, scaled_penalty_disabled, lambda_dp_able, lambda_dp_disabled
