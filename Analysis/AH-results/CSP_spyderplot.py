...
def generate_radar_plot(selected_algorithms, plot_title, output_filename, group_name):
    """
    Loads data for a given set of algorithms and a specific CSP group,
    then generates a radar plot
    """
    data = {}
    all_actions = set()

    group_key_lower = group_name.lower()
    ns_key = f"non_sensitive_{group_key_lower}"
    se_key = f"sensitive_{group_key_lower}"
    ...
    for algo, knob in selected_algorithms.items():
        ...
        freqs = store[knob].get("action_frequencies", {})
        if not {ns_key, se_key} <= freqs.keys():
            print(f"[warn] {algo} missing group keys ('{ns_key}', '{se_key}') for '{group_name}' group – skipped")
            continue
        ...
        ns, se = normalise(freqs[ns_key]), normalise(freqs[se_key])
...
if __name__ == "__main__":
    ...
    for group in ["Red", "Blue"]:
        generate_radar_plot(
            ...
            plot_title=f"Action Profiles of Fair-PPO Variants ({group} Group)",
            output_filename=f"fair_ppo_variants_radar_{group.lower()}.svg",
            group_name=group
        )

        generate_radar_plot(
            selected_algorithms=other_algs_selected,
            plot_title=f"Action Profiles of Baseline Algorithms ({group} Group)",
            output_filename=f"baseline_algorithms_radar_{group.lower()}.svg",
            group_name=group
        )
